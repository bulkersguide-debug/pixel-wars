...
const PRODUCTS = {
  ...
  powerup_bomb:     { name: "Cluster Bomb",        pixels: 0,    bonus: 0,    price: 1000 },
  powerup_storm:    { name: "Pixel Storm",         pixels: 0,    bonus: 0,    price: 1500 },
  powerup_fortress: { name: "Fortress",            pixels: 0,    bonus: 0,    price: 800  },
  powerup_snipe:    { name: "Sniper",              pixels: 0,    bonus: 0,    price: 300  },
  powerup_airdrop:  { name: "Airdrop",             pixels: 0,    bonus: 0,    price: 2500 },
  powerup_nuke:     { name: "Nuke",                pixels: 0,    bonus: 0,    price: 2500 },
  powerup_renew:    { name: "Renewal Shield",      pixels: 0,    bonus: 0,    price: 500  },
  powerup_double:   { name: "Double or Nothing",   pixels: 0,    bonus: 0,    price: 500  },
  powerup_surge:    { name: "Surge",               pixels: 0,    bonus: 0,    price: 600  },
};
...
export default async function handler(req, res) {
  ...
  const { productId, userId, discordUsername, fandom, name, description, amount, metadata } = req.body;
  ...
  const isPowerup = productId.startsWith("powerup_");
  const totalPixels = product.pixels + product.bonus;
  const finalPrice = amount || product.price;
  ...
  try {
    ...
    params.append("line_items[0][price_data][product_data][name]", `${name || product.name} — Pixels of War`);
    params.append("line_items[0][price_data][product_data][description]", description || (isPowerup ? `Power-up: ${product.name}` : `${totalPixels} pixels added to your account`));
    ...
    params.append("metadata[fandom]", fandom || metadata?.fandom || "none");
    params.append("metadata[pixels]", String(totalPixels));
    params.append("metadata[isPowerup]", isPowerup ? "true" : "false");
    params.append("metadata[powerupId]", metadata?.powerupId || "none");
