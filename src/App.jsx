...
// Module-level season number — updated when season loads, used in pixel saves
...
async function dbUpsertPixels(pending,teamId,sn){
  ...
  console.log(`[pixels] saving ${rows.length}px to season ${sn} for ${teamId}`);
  for(let i=0;i<rows.length;i+=500){
    const{error}=await supabase.from("pixels").upsert(rows.slice(i,i+500),{onConflict:"idx,season_num"});
    if(error)console.error("[pixels] save error:",JSON.stringify(error));
    else console.log(`[pixels] batch ${i/500+1} saved OK`);
  }
}
...
export default function App(){
  ...
  const spawnFX=useCallback((gx,gy,color,rgb,isRaid=false)=>{
    ...
    const idx=gy*GW+gx;fxHeat.current[idx]=1.5;
    const n=isRaid?16:8;
    for(let i=0;i<n;i++){
      const a=Math.random()*Math.PI*2,s=isRaid?(2+Math.random()*4):(1+Math.random()*2.5);
      fxParticles.current.push({x:cx2,y:cy2,vx:Math.cos(a)*s,vy:Math.sin(a)*s-1,life:1,size:isRaid?3.5+Math.random()*2:2+Math.random()*2,r:rgb[0],g:rgb[1],b:rgb[2],grav:0.08});
    }
    if(isRaid){
      fxExplosions.current.push({x:cx2,y:cy2,er:0,life:1.2,cr:rgb[0],cg:rgb[1],cb:rgb[2]});
      fxExplosions.current.push({x:cx2,y:cy2,er:eC,life:0.8,cr:255,cg:200,cb:0});
      fxFloats.current.push({x:cx2,y:cy2-eC,vy:-1.2,text:"⚔",life:1.5,color});
      for(let i=0;i<5;i++){
        const a=Math.random()*Math.PI*2,len=eC*(3+Math.random()*5);
        fxBeams.current.push({x1:cx2,y1:cy2,x2:cx2+Math.cos(a)*len,y2:cy2+Math.sin(a)*len,life:1.2,color});
      }
    }else{
      fxExplosions.current.push({x:cx2,y:cy2,er:0,life:0.9,cr:rgb[0],cg:rgb[1],cb:rgb[2]});
    }
  },[]);
  ...
  // Fast tick — always runs for alive grid FX
  useEffect(()=>{
    let raf;const tick=()=>{
      setAnimTick(t=>t+1);
  ...
  useEffect(()=>{
    async function load(){
      ...
      if(isOnline){
        ...
        let sNum=1;
        if(dbSeason){const sObj={num:dbSeason.num,startDate:dbSeason.start_date,theme:dbSeason.theme_index,winners:dbSeason.winners||[]};setSeason(sObj);sNum=dbSeason.num;ACTIVE_SEASON_NUM=sNum;console.log("[season] active season:",sNum);}
  ...
  useEffect(()=>{
    ...
    // ── ALIVE GRID FX ──────────────────────────────────────────────────────────
    fxTick.current++;
    const ft2=fxTick.current*0.04;
    // Decay heat
    Object.keys(fxHeat.current).forEach(k=>{fxHeat.current[k]=Math.max(0,fxHeat.current[k]-0.03);if(fxHeat.current[k]<=0)delete fxHeat.current[k];});
    // Heat overlay on recently claimed pixels
    Object.entries(fxHeat.current).forEach(([idxStr,h])=>{
      const idx=parseInt(idxStr),gx2=idx%GW,gy2=Math.floor(idx/GW);
      const sx2=(gx2-vx)*eC,sy2=(gy2-vy)*eC;
      if(sx2<-eC||sx2>CW||sy2<-eC||sy2>CH)return;
      ctx.fillStyle=`rgba(255,180,0,${h*0.6})`;ctx.fillRect(sx2,sy2,eC,eC);
    });
    // Energy beams
    for(let i=fxBeams.current.length-1;i>=0;i--){
      const b=fxBeams.current[i];b.life-=0.06;
      if(b.life<=0){fxBeams.current.splice(i,1);continue;}
      ctx.save();ctx.shadowColor=b.color;ctx.shadowBlur=8;
      ctx.strokeStyle=b.color+(Math.floor(b.life*255).toString(16).padStart(2,"0"));
    ...
    for(let i=fxExplosions.current.length-1;i>=0;i--){
      const ex=fxExplosions.current[i];ex.er+=2.5;ex.life-=0.04;
      if(ex.life<=0){fxExplosions.current.splice(i,1);continue;}
      ctx.save();ctx.shadowColor=`rgb(${ex.cr},${ex.cg},${ex.cb})`;ctx.shadowBlur=6;
      ctx.strokeStyle=`rgba(${ex.cr},${ex.cg},${ex.cb},${ex.life})`;
      ctx.lineWidth=2.5;ctx.beginPath();ctx.arc(ex.x,ex.y,ex.er,0,Math.PI*2);ctx.stroke();
      ctx.restore();
    }
    // Particles
    for(let i=fxParticles.current.length-1;i>=0;i--){
      const p=fxParticles.current[i];p.x+=p.vx;p.y+=p.vy;p.vy+=p.grav;p.life-=0.025;
    ...
    for(let i=fxFloats.current.length-1;i>=0;i--){
      const ft3=fxFloats.current[i];ft3.y+=ft3.vy;ft3.life-=0.02;
      if(ft3.life<=0){fxFloats.current.splice(i,1);continue;}
      ctx.font=`bold ${10+ft3.life*4}px monospace`;ctx.textAlign="center";
      ctx.fillStyle=`rgba(0,0,0,${ft3.life*0.6})`;ctx.fillText(ft3.text,ft3.x+1,ft3.y+1);
      ctx.fillStyle=ft3.color+(Math.floor(ft3.life*255).toString(16).padStart(2,"0"));
      ctx.fillText(ft3.text,ft3.x,ft3.y);
    }
    // Scanline sweep
    fxScan.current=(fxScan.current+0.8)%CH;
    ctx.fillStyle="rgba(0,220,255,0.04)";ctx.fillRect(0,fxScan.current,CW,2);
    // Territory pulse — animated glowing border on YOUR pixels
    if(active&&at){
      const[r2,g2,b2]=[parseInt(at.color.slice(1,3),16),parseInt(at.color.slice(3,5),16),parseInt(at.color.slice(5,7),16)];
      ctx.save();ctx.shadowColor=at.color;ctx.shadowBlur=6;
      for(let dy=0;dy<effVH;dy++){for(let dx=0;dx<effVW;dx++){
        ...
        const hasN=pixels[(gy2-1)*GW+gx2]?.t===active;
        const hasS=pixels[(gy2+1)*GW+gx2]?.t===active;
        const hasW=pixels[gy2*GW+(gx2-1)]?.t===active;
        const hasE=pixels[gy2*GW+(gx2+1)]?.t===active;
        if(hasN&&hasS&&hasW&&hasE){
          // Interior shimmer — bright white flash
          const shim=Math.sin(ft2*2.5+gx2*0.6+gy2*0.5)*0.5+0.5;
          if(shim>0.7){ctx.fillStyle=`rgba(255,255,255,${(shim-0.7)*0.25})`;ctx.fillRect(dx*eC+1,dy*eC+1,eC-2,eC-2);}
        }else{
          // Glowing border — strong visible pulse
          const pulse2=0.5+0.5*Math.sin(ft2*3+gx2*0.5+gy2*0.4);
          const a2=0.5+pulse2*0.5;
          if(!hasN){ctx.fillStyle=`rgba(${r2},${g2},${b2},${a2})`;ctx.fillRect(dx*eC,dy*eC,eC,2);}
          if(!hasS){ctx.fillStyle=`rgba(${r2},${g2},${b2},${a2*0.7})`;ctx.fillRect(dx*eC,dy*eC+eC-2,eC,2);}
          if(!hasW){ctx.fillStyle=`rgba(${r2},${g2},${b2},${a2})`;ctx.fillRect(dx*eC,dy*eC,2,eC);}
          if(!hasE){ctx.fillStyle=`rgba(${r2},${g2},${b2},${a2*0.7})`;ctx.fillRect(dx*eC+eC-2,dy*eC,2,eC);}
        }
      }}
      ctx.restore();
    }
  ...
  const requestClaim=()=>{
    ...
    if(!user){
      ...
      if(isOnline)dbUpsertPixels(allowedSet,active,ACTIVE_SEASON_NUM);
    ...
    <div style={{background:"#040408",minHeight:"100vh",fontFamily:"'Rajdhani',sans-serif",color:"#e0e8ff",userSelect:"none",position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{background:"linear-gradient(90deg,rgba(0,245,255,.1),rgba(200,255,0,.05))",border:"1px solid rgba(0,245,255,.25)",borderRadius:10,padding:"14px",textAlign:"center",marginBottom:10}}>
                    ...
                    <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:8}}>
                      <div style={{textAlign:"center"}}>
                        ...
                        <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:7,color:"rgba(255,255,255,.3)"}}>AVAILABLE</div>
                      </div>
                      <div style={{textAlign:"center"}}>
      ...
      {showWorldWar&&(()=>{
        ...
        const doFactionRaid=()=>{
          ...
          setPixels(next);
          if(isOnline)dbUpsertPixels(new Set(toUpsert),active,ACTIVE_SEASON_NUM);
      ...
      <div style={{background:"#06060e",borderBottom:"1px solid #1a1a30",padding:"7px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
          {freePixels>0&&<div style={{display:"flex",alignItems:"center",gap:5,background:"rgba(255,215,0,.08)",border:"1px solid rgba(255,215,0,.3)",borderRadius:7,padding:"4px 9px",cursor:"pointer"}} onClick={gatedOpenDaily}>
            <span style={{fontSize:13}}>🎁</span><div><div style={{fontFamily:"'Orbitron',monospace",fontSize:10,fontWeight:900,color:"#FFD700"}}>{freePixels+(rechargePixels||0)}px</div><div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:6,color:"#5a5a5a"}}>{rechargePixels>0?`+${rechargePixels}⏰`:alreadyClaimedToday?"CLAIMED":"TAP"}</div></div>
          </div>}
      ...
      {showWarChest&&active&&<div style={{background:"rgba(6,6,18,.97)",borderBottom:"1px solid rgba(255,215,0,.25)",padding:"10px 14px",animation:"slideDown .2s ease"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <button onClick={()=>{
            ...
            setPixels(next);
            if(isOnline)dbUpsertPixels(new Set(toUpsert2),active,ACTIVE_SEASON_NUM);
            const newGold=warChest-20;setWarChest(newGold);localStorage.setItem("pow_war_chest",String(newGold));
            spawnParticles("#FFD700",vx+VW/2,vy+VH/2,20,true);triggerFlash("#FFD700",true);
