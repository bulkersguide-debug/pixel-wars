...
const EVENTS=[
  ...
  {icon:"🔓",label:"SECTOR SALE",desc:"ALL sector prices halved 10 min!",duration:600,color:"#00AAFF"},
];
...
export default function App(){
  ...
  const claimDaily=()=>{
    ...
    const nf=freePixels+dailyInfo.reward.px;setFreePixels(nf);localStorage.setItem("pow_free",String(nf));
  ...
  return(
    <div style={{background:"#040408",minHeight:"100vh",fontFamily:"'Rajdhani',sans-serif",color:"#e0e8ff",userSelect:"none",position:"relative",overflow:"hidden"}}>
      ...
      {/* DAILY MODAL */}
      {showDaily&&dailyInfo&&!alreadyClaimedToday&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,backdropFilter:"blur(10px)"}}>
        <div style={{background:"#09091c",border:"1px solid rgba(255,215,0,.4)",borderRadius:16,padding:"30px 26px",width:380,maxWidth:"94vw",animation:"pop .4s cubic-bezier(.34,1.56,.64,1)",textAlign:"center"}}>
          ...
          <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:10,color:"#5a5a7a",marginBottom:18,letterSpacing:1}}>DAY {dailyInfo.days} STREAK</div>
        ...
        <div style={{background:"#09091c",border:"1px solid rgba(255,215,0,.5)",borderRadius:20,padding:"36px 32px",width:440,maxWidth:"94vw",animation:"pop .5s cubic-bezier(.34,1.56,.64,1)",textAlign:"center"}}>
          ...
          <div style={{background:"rgba(200,255,0,.06)",border:"1px solid rgba(200,255,0,.2)",borderRadius:10,padding:"12px 16px",marginBottom:20,display:"flex",gap:12,alignItems:"center"}}>
            ...
            <div style={{textAlign:"left"}}>
              <div style={{fontFamily:"'Orbitron',monospace",fontSize:12,fontWeight:900,color:"#C8FF00"}}>SEASON {season.num+1}: {THEMES[(season.theme+1)%THEMES.length].name}</div>
      ...
      <div style={{background:"#06060e",borderBottom:"1px solid #1a1a30",padding:"7px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
          {freePixels>0&&<div style={{display:"flex",alignItems:"center",gap:5,background:"rgba(255,215,0,.08)",border:"1px solid rgba(255,215,0,.3)",borderRadius:7,padding:"4px 9px",cursor:"pointer"}} onClick={openDailyModal}>
            ...
            <div><div style={{fontFamily:"'Orbitron',monospace",fontSize:10,fontWeight:900,color:"#FFD700"}}>{freePixels}px FREE</div><div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:6,color:"#5a5a5a"}}>{alreadyClaimedToday?"CLAIMED":"TAP"}</div></div>
          </div>}
          <div style={{display:"flex",alignItems:"center",gap:5,background:"rgba(255,45,120,.07)",border:"1px solid rgba(255,45,120,.25)",borderRadius:7,padding:"4px 9px",cursor:"pointer"}} onClick={openDailyModal}>
            <span style={{fontSize:13}}>🔥</span>
            <div><div style={{fontFamily:"'Orbitron',monospace",fontSize:10,fontWeight:900,color:"#FF2D78"}}>{streakData.days}d</div><div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:6,color:"#5a5a5a"}}>{alreadyClaimedToday?"✅":"STREAK"}</div></div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:5,background:rgba(myRank.color,.07),border:`1px solid ${rgba(myRank.color,.25)}`,borderRadius:7,padding:"4px 9px"}}>
            <span style={{fontSize:13}}>{myRank.icon}</span>
            <div><div style={{fontFamily:"'Orbitron',monospace",fontSize:10,fontWeight:900,color:myRank.color}}>{myRank.name}</div><div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:6,color:"#5a5a5a"}}>{myPixels}px</div></div>
          </div>
        </div>
        <div style={{display:"flex",gap:14,flexWrap:"wrap",alignItems:"center"}}>
          {[["SOLD",totalSold.toLocaleString(),"#00F5FF"],["€ REV",`€${totalSold}`,"#C8FF00"],["SECTORS",`${unlockedSectors.length}/400`,"#FF2D78"]].map(([l,v,c])=>(
            <div key={l} style={{textAlign:"right"}}>
              ...
              <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:7,color:"#2a2a4a",letterSpacing:1}}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{display:"flex",height:`calc(100vh - ${140+(event?28:0)}px)`,overflow:"hidden"}}>
            ...
            {mode==="SHOP"?(
              ...
              <>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(118px,1fr))",gap:3}}>
                  {vis.map(t=>{const isA=active===t.id,acc=CAT_ACCENT[t.cat]||"#00F5FF";const px=Object.values(pixels).filter(p=>p?.t===t.id).length;const rank=getRank(px);return(
                    <div key={t.id} className="tbtn" onClick={()=>setActive(isA?null:t.id)} style={{padding:"5px 7px",borderRadius:5,cursor:"pointer",border:`1px solid ${isA?t.color:acc+"18"}`,background:isA?rgba(t.color,.1):"#0c0c1a",transition:"all .1s",position:"relative",overflow:"hidden"}}>
                      ...
                      {px>0&&<div style={{display:"flex",alignItems:"center",gap:2,marginTop:1}}><span style={{fontSize:7}}>{rank.icon}</span><span style={{fontSize:7,color:rank.color,fontFamily:"'Orbitron',monospace",fontWeight:700}}>{px}px</span></div>}
                    </div>
                  );})}
                </div>
              </>
        ...
        {/* RIGHT PANEL */}
        <div style={{width:185,borderLeft:"1px solid #1a1a30",background:"#05050d",flexShrink:0,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          {tab==="WAR"?(
            ...
            <div style={{flex:1,overflowY:"auto",padding:"5px 5px"}}>
              ...
              {feed.map(f=>(
                <div key={f.id} style={{marginBottom:3,padding:"3px 5px",background:f.isMe?rgba(f.color,.08):"#08081a",borderRadius:4,border:`1px solid ${f.isMe?rgba(f.color,.3):"#1a1a2a"}`,animation:"slideDown .2s ease"}}>
                  <div style={{display:"flex",gap:3,alignItems:"flex-start"}}>
                    ...
                    <div style={{minWidth:0}}>
                      ...
                      <span style={{fontSize:7,color:"#5a5a7a"}}>{f.msg}</span>
